...
function productRow(clientId, product) {
  ...
  return `<div class="product">
    <div class="product-top">
      <div class="product-name">${esc(product.name)}<div class="muted">${esc(product.unit)}</div></div>
      <div class="qty">
    ...
    <div class="orders-shell">
      ...
      <div>
        ...
        <article class="card clean-section">
          <div class="section-title">
            <div><h2>${esc(client.name)}</h2><div class="muted">${orderLines(client.id).length} productos</div></div>
        ...
        <article class="card">
          <div id="productList" class="compact-product-grid">
...
function renderBuy() {
  ...
  $("#view-buy").innerHTML = `
    <article class="card">
      <div class="section-title">
        <div><h2>Compra</h2><div class="muted">${list.length} productos</div></div>
    ...
    <article class="card">
      ${list.map((item) => {
        ...
        const bought = state.route.purchasedQty?.[item.product.id] === undefined ? totalToBuy : num(state.route.purchasedQty[item.product.id]);
        return `<div class="simple-buy-row">
          <div>
            <b>${esc(item.product.name)}</b>
            <div class="meta">Pedido ${fmt(item.qty)} · Extra ${fmt(extra)} · Comprar ${fmt(totalToBuy)} ${esc(item.product.unit)}</div>
          </div>
...
function buyDetailModal(productId) {
  ...
  const delivered = clients(true).reduce((sum,c) => sum + finalQty(c.id,productId), 0);
  const stock = bought - delivered;
  openModal("Detalle de compra", `
    <article class="card">
      <h2>${esc(item.product.name)}</h2>
      <div class="line"><span>Pedido clientes</span><b>${fmt(item.qty)} ${esc(item.product.unit)}</b></div>
      <div class="line"><span>Extra</span><input class="field" style="width:90px" data-extra-input="${productId}" value="${fmt(extra)}"></div>
      <div class="line"><span>Comprado real</span><input class="field" style="width:90px" data-bought-input="${productId}" value="${fmt(bought)}"></div>
      <div class="line"><span>Entregado</span><b>${fmt(delivered)}</b></div>
      <div class="line"><span>Stock restante</span><b>${fmt(stock)}</b></div>
...
function renderDelivery() {
  ...
  if (!routeClients.length) {
    $("#view-delivery").innerHTML = `
      <article class="card">
        <div class="section-title"><h2>Reparto</h2><button class="btn soft small" data-action="toggle-delivery-completed">Mostrar completados</button></div>
        <div class="muted">No quedan clientes pendientes.</div>
      </article>`;
    return;
  }
  ...
  $("#view-delivery").innerHTML = `
    <div class="delivery-focus">
      ...
      <article class="card">
        <div class="section-title">
          <div><h2>${esc(client.name)}</h2><div class="muted">${done}/${lines.length} bajados</div></div>
