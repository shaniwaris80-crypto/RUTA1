...
function renderBuy() {
  ...
  const filtered = list.filter((item)=>{
    ...
    const expected=item.qty+extra;
    const bought=state.route.purchasedQty?.[item.product.id]===undefined?expected:num(state.route.purchasedQty[item.product.id]);
    const stock=stockForProduct(item.product.id);
    const status=purchaseStatusFor(item.product.id,expected,bought);
    if (purchaseProviderFilter!=="Todos" && supplierName(item.product)!==purchaseProviderFilter) return false;
  ...
  $("#view-buy").innerHTML = `
    <div class="final-shell">
      <section class="page-title">
        <span class="overline">Control de compra</span>
        <h2>Compras y stock</h2>
        <p>Marca lo comprado y revisa las diferencias sin abrir varias pantallas.</p>
      </section>

      <section class="metric-grid">
        <div><b>${stats.products}</b><span>Productos</span></div>
        <div><b>${fmt(stats.ordered)}</b><span>Total comprar</span></div>
        <div><b>${fmt(stats.stock)}</b><span>Stock</span></div>
        <div><b>${fmt(stats.shortage)}</b><span>Faltantes</span></div>
      </section>
      ...
      <section class="filters-final">
        ...
        <div class="mode-pills">
          ${[["all","Todos"],["pending","Pendientes"],["ordered","Pedidos"],["bought","Comprados"],["partial","Parciales"],["missing","No conseguido"],["stock","Stock"],["shortage","Faltantes"]]
            .map(([key,label])=>`<button class="${purchaseFilter===key?"active":""}" data-purchase-filter="${key}">${label}</button>`).join("")}
        </div>
      </section>

      <section class="purchase-list-final">
        ${filtered.map((item)=>{
          const extra=num(state.route.extraPurchase?.[item.product.id]);
          const expected=item.qty+extra;
          const bought=state.route.purchasedQty?.[item.product.id]===undefined?expected:num(state.route.purchasedQty[item.product.id]);
          const status=purchaseStatusFor(item.product.id,expected,bought);
          const stock=stockForProduct(item.product.id);
          return `<div class="purchase-row-final">
            <button class="purchase-product" data-product-info="${item.product.id}">
              <b>${esc(item.product.name)}</b>
              <span>${esc(supplierName(item.product))} · ${esc(item.product.unit)}</span>
              <small>Pedido ${fmt(item.qty)} · Extra ${fmt(extra)} · Total ${fmt(expected)}</small>
            </button>
            <div class="purchase-input-block">
              <label>Comprado</label>
              <input data-bought-input="${item.product.id}" inputmode="decimal" value="${fmt(bought)}" ${routeIsClosed()?"disabled":""}>
            </div>
            <div class="purchase-result">
              <span class="status-dot ${purchaseStatusClass(status)}">${purchaseStatusLabel(status)}</span>
              <small>${stock>0?`Stock ${fmt(stock)}`:stock<0?`Faltan ${fmt(Math.abs(stock))}`:"Cuadrado"}</small>
            </div>
            <div class="row-buttons">
              <button class="outline-button" data-buy-detail="${item.product.id}">Detalle</button>
              <button class="black-button" data-mark-bought="${item.product.id}" ${routeIsClosed()?"disabled":""}>✓</button>
            </div>
          </div>`;
...
async async function saveProduct(productId="") {
