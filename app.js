...
function renderHome() {
  ...
  const completed = visibleClients.filter((c) => state.route.status?.[c.id] === "done").length;
  const skipped = clients(true).filter((c) => state.route.status?.[c.id] === "skip").length;
  const ps = purchaseStats();
  const pendingCount = deadlineStatus().pending.length;

  $("#view-home").innerHTML = `
    <div class="pro-shell">
      <article class="card hero-pro">
        <div class="hero-copy">
          <div class="eyebrow">Panel general</div>
          <h2>Ruta Madrid</h2>
          <div class="section-subtle">Todo el control de la semana en una sola vista.</div>
        </div>
        <div class="hero-meta">
          <div class="hero-date">${state.route.date}</div>
          <button class="btn primary" data-action="go-orders">Ir a pedidos</button>
        </div>
        <div class="pro-kpis">
          <div class="kpi-card"><b>${visibleClients.length}</b><span>Clientes activos</span></div>
          <div class="kpi-card"><b>${completed}</b><span>Pedidos hechos</span></div>
          <div class="kpi-card"><b>${skipped}</b><span>No pide</span></div>
          <div class="kpi-card"><b>${pendingCount}</b><span>Pendientes</span></div>
        </div>
      </article>
      ...
      <div class="pro-grid-2">
        <article class="card">
          <div class="clean-header">
            <div><h2>Compra y stock</h2><div class="section-subtle">Resumen rápido y limpio</div></div>
            <button class="btn soft small" data-view="buy">Abrir</button>
          </div>
          <div class="pro-kpis mini">
            <div class="kpi-card"><b>${ps.products}</b><span>Productos</span></div>
            <div class="kpi-card"><b>${fmt(ps.ordered)}</b><span>Total comprar</span></div>
            <div class="kpi-card"><b>${fmt(ps.stock)}</b><span>Stock</span></div>
            <div class="kpi-card"><b>${fmt(ps.shortage)}</b><span>Faltantes</span></div>
          </div>
        </article>

        <article class="card deadline-card ${deadlineStatus().ms <= 0 ? "closed" : pendingCount ? "" : "ok"}">
          <div class="clean-header">
            <div><h2>Plazo de pedidos</h2><div class="section-subtle">Jueves hasta las 23:00</div></div>
            <button class="btn soft small" data-action="pending-orders">Ver</button>
          </div>
          <div id="deadlineCountdown" class="deadline-countdown">${formatCountdown(deadlineStatus().ms)}</div>
          <div class="section-subtle">${pendingCount} clientes siguen pendientes</div>
          <div class="focus-actions" style="margin-top:10px">
            <button class="btn soft" data-action="copy-pending">Copiar aviso</button>
            <button class="btn dark" data-action="share-pending">WhatsApp</button>
          </div>
        </article>
      </div>
      ...
      <article class="card">
        <div class="clean-header">
          <div><h2>Gestión</h2><div class="section-subtle">Opciones generales</div></div>
        </div>
        <div class="quick-grid">
          <button class="quick-action" data-action="add-client"><span>＋</span><b>Añadir cliente</b><small>Crear nuevo cliente</small></button>
          <button class="quick-action" data-action="add-product"><span>▦</span><b>Añadir producto</b><small>Ampliar catálogo</small></button>
          <button class="quick-action" data-action="show-history"><span>↺</span><b>Historial</b><small>Rutas anteriores</small></button>
          <button class="quick-action" data-action="export-backup"><span>⤓</span><b>Exportar copia</b><small>Guardar respaldo</small></button>
          <button class="quick-action" data-action="import-backup"><span>⤒</span><b>Restaurar copia</b><small>Cargar respaldo</small></button>
          <button class="quick-action danger" data-action="new-route"><span>⇢</span><b>Nueva ruta</b><small>Limpiar la semana</small></button>
        </div>
      </article>
    </div>`;
}
  ...
  openModal("Detalle del producto", `
    <article class="card">
      <div class="clean-header">
        ...
        <button class="btn soft small" data-buy-detail="${productId}">Compra</button>
      </div>
...
function productRow(clientId, product) {
  ...
  return `<div class="product">
    <div class="product-touch">
      <button class="product-link" data-product-info="${product.id}">
        <span class="product-title">${esc(product.name)}</span>
        <span class="muted">${esc(product.unit)} · tocar para ver quién lo tiene</span>
      </button>
      <div class="qty">
...
function renderOrders() {
  ...
  if (!client) {
    $("#view-orders").innerHTML = '<article class="card pro-empty"><div class="muted">No hay clientes pendientes.</div></article>';
    return;
  }
  ...
  $("#view-orders").innerHTML = `
    <div class="pro-shell orders-shell">
      <aside class="card tablet-client-list tablet-panel">
        <div class="clean-header">
          <div><h2>Clientes</h2><div class="section-subtle">${allClients.length} activos</div></div>
          <span class="inline-chip">${completedClientsText()}</span>
        </div>
        <div class="compact-grid compact-client-list">
          ${allClients.map((c) => {
            const active = c.id === selectedClientId;
            const done = state.route.status?.[c.id] === "done";
            return `<button class="client-pill ${active ? "active" : ""}" data-open-client="${c.id}">
              <span>${fmt(c.routeOrder)}. ${esc(c.name)}</span><small>${done ? "Hecho" : "Pendiente"}</small>
            </button>`;
          }).join("")}
        </div>
      </aside>
      ...
      <div class="pro-main">
        <div class="mobile-order-header">
          <div class="order-client-bar top-chooser">
            <button class="btn soft" data-prev-client>←</button>
            ...
            <button class="btn soft" data-next-client>→</button>
          </div>
        </div>

        <article class="card hero-subtle">
          <div class="clean-header">
            <div><h2>${esc(client.name)}</h2><div class="section-subtle">Cliente ${currentIndex + 1} de ${allClients.length}</div></div>
            <button class="btn soft small" data-action="toggle-order-actions">Acciones</button>
          </div>
          <div class="pro-kpis mini" style="margin-top:10px">
            <div class="kpi-card"><b>${lines.length}</b><span>Productos</span></div>
            <div class="kpi-card"><b>${fmt(totalQty)}</b><span>Total</span></div>
            <div class="kpi-card"><b>${productList.length}</b><span>Visibles</span></div>
            <div class="kpi-card"><b>${deadlineStatus().pending.length}</b><span>Pendientes</span></div>
          </div>

          <div class="secondary-actions ${orderActionsOpen ? "open" : ""}">
          ...
          <input id="productSearch" class="search search-pro" style="margin-top:10px" placeholder="Buscar producto" value="${esc(productSearch)}">
          <div class="tabs">
            ${categories.map((cat) => `<button class="btn ${cat === productCategory ? "dark" : "soft"} small" data-category="${esc(cat)}">${esc(cat)}</button>`).join("")}
          </div>
        </article>

        <article class="card pro-products">
          <div class="tap-hint">Toca el nombre del producto para ver quiénes lo tienen y cuánto.</div>
          <div id="productList" class="compact-product-grid" style="margin-top:10px">
            ${productList.map((p) => productRow(client.id,p)).join("") || '<div class="muted">Sin resultados</div>'}
          </div>
        </article>

        <button class="btn primary big-next dock-save" data-save-next="${client.id}|${currentIndex}">Guardar y siguiente</button>
      </div>
    </div>`;
}
...
function purchaseStatusFor(productId, orderedQty, boughtQty) {
  ...
  if (boughtQty <= 0) return "pending";
  if (boughtQty < orderedQty) return "partial";
  return "bought";
}
...
function purchaseStats() {
  const list = totals();
  let ordered = 0, bought = 0, stock = 0, shortage = 0;
  for (const item of list) {
    const extra = num(state.route.extraPurchase?.[item.product.id]);
    const expected = item.qty + extra;
    const real = state.route.purchasedQty?.[item.product.id] === undefined ? expected : num(state.route.purchasedQty[item.product.id]);
  ...
  return { products:list.length, ordered, bought, stock, shortage };
}
...
function renderBuy() {
  ...
  const filtered = list.filter((item) => {
    ...
    if (purchaseFilter === "stock") return stock > 0;
    if (purchaseFilter === "shortage") return stock < 0;
    return status === purchaseFilter;
  });

  $("#view-buy").innerHTML = `
    <div class="pro-shell narrow-shell">
      <article class="card hero-subtle">
        <div class="clean-header">
          <div><h2>Compras</h2><div class="section-subtle">Vista clara para comprar y controlar stock</div></div>
          <button class="btn soft small" data-action="copy-buy">Copiar</button>
        </div>
        <div class="purchase-dashboard" style="margin-top:10px">
          <div class="purchase-metric"><b>${stats.products}</b><span>Productos</span></div>
          <div class="purchase-metric"><b>${fmt(stats.ordered)}</b><span>Total comprar</span></div>
          <div class="purchase-metric"><b>${fmt(stats.stock)}</b><span>Stock</span></div>
          <div class="purchase-metric"><b>${fmt(stats.shortage)}</b><span>Faltantes</span></div>
        </div>
      </article>
      ...
      <article class="card">
        <div class="purchase-filters">
          ${[
            ["all","Todos"],["pending","Pendientes"],["ordered","Pedidos"],["bought","Comprados"],
            ["partial","Parciales"],["missing","No conseguido"],["stock","Con stock"],["shortage","Con faltante"]
          ].map(([key,label])=>`<button class="btn ${purchaseFilter===key?"dark":"soft"} small purchase-filter" data-purchase-filter="${key}">${label}</button>`).join("")}
        </div>
        ...
        ${filtered.map((item) => {
          ...
          const stock = bought - delivered;
          const status = purchaseStatusFor(item.product.id, expected, bought);
          return `<div class="purchase-card premium">
            <button class="product-link purchase-main" data-product-info="${item.product.id}">
              <b>${esc(item.product.name)}</b>
              <div class="purchase-meta">Pedido ${fmt(item.qty)} · Extra ${fmt(extra)} · Comprar ${fmt(expected)} ${esc(item.product.unit)}</div>
              <div class="purchase-inline">
                <span class="purchase-status ${purchaseStatusClass(status)}">${purchaseStatusLabel(status)}</span>
                <span class="ghost-pill">${stock>0?`Stock ${fmt(stock)}`:stock<0?`Faltan ${fmt(Math.abs(stock))}`:"Cuadrado"}</span>
              </div>
            </button>
            <div class="purchase-actions vertical">
              <button class="btn soft small" data-buy-detail="${item.product.id}">Abrir</button>
              <button class="btn ${status==="bought"?"dark":"primary"} small" data-mark-bought="${item.product.id}">${status==="bought"?"Comprado":"Marcar comprado"}</button>
            </div>
          </div>`;
        }).join("") || '<div class="muted">No hay productos en este filtro.</div>'}
      </article>
    </div>`;
}
...
function buyDetailModal(productId) {
  ...
  const stock = bought - delivered;
  const status = purchaseStatusFor(productId, expected, bought);
  openModal("Compra y stock", `
    <article class="card center-card">
      <div class="clean-header">
        <div><h2>${esc(item.product.name)}</h2><div class="section-subtle">${esc(item.product.unit)}</div></div>
        <span class="purchase-status ${purchaseStatusClass(status)}">${purchaseStatusLabel(status)}</span>
      </div>
      <div class="purchase-detail-grid" style="margin-top:10px">
        ...
        <div class="purchase-detail-item"><span>Extra</span><input class="field" data-extra-input="${productId}" value="${fmt(extra)}"></div>
        <div class="purchase-detail-item"><span>Total previsto</span><b>${fmt(expected)}</b></div>
        <div class="purchase-detail-item"><span>Comprado real</span><input class="field" data-bought-input="${productId}" value="${fmt(bought)}"></div>
        <div class="purchase-detail-item"><span>Repartido</span><b>${fmt(delivered)}</b></div>
        <div class="purchase-detail-item"><span>${stock>=0?"Stock":"Faltante"}</span><b>${fmt(Math.abs(stock))}</b></div>
      </div>
      <div class="status-buttons">
        <button class="btn soft" data-set-purchase-status="${productId}|ordered">Pedido</button>
      ...
      <button class="btn dark" style="width:100%;margin-top:10px" data-allocate-product="${productId}">Repartir entre clientes</button>
    </article>
    <article class="card center-card">
      <div class="clean-header"><h2>Clientes</h2><div class="tap-hint">${rows.length}</div></div>
      <div class="modal-list">
        ${rows.map((row) => `<div class="modal-row">
          <div><b>${esc(row.client.name)}</b><div class="section-subtle">${fmt(row.ordered)} ${esc(item.product.unit)}</div></div>
          <button class="btn soft small" data-open-client="${row.client.id}">Abrir</button>
        </div>`).join("")}
      </div>
    </article>`);
...
function renderLoad() {
  ...
  $("#view-load").innerHTML = `
    <div class="pro-shell narrow-shell">
      <article class="card hero-subtle">
        <div class="clean-header">
          <div><h2>Carga</h2><div class="section-subtle">Control simple antes de salir</div></div>
          <button class="btn soft small" data-action="reset-load">Reiniciar</button>
        </div>
        <div class="pro-kpis mini" style="margin-top:10px">
          <div class="kpi-card"><b>${list.length}</b><span>Productos</span></div>
          <div class="kpi-card"><b>${done.length}</b><span>Cargados</span></div>
          <div class="kpi-card"><b>${pending.length}</b><span>Pendientes</span></div>
          <div class="kpi-card"><b>${pct}%</b><span>Progreso</span></div>
        </div>
      ...
      <article class="card">
        ${[...pending,...done].map((item) => {
          ...
          return `<div class="load-touch-row premium">
            <button class="product-link" data-product-info="${item.product.id}">
              <span class="product-title">${esc(item.product.name)}</span>
              <span class="muted">${fmt(bought)} ${esc(item.product.unit)}</span>
            </button>
            <button class="btn ${checked ? "dark" : "soft"} small" data-toggle-load="${item.product.id}">${checked ? "Cargado" : "Marcar cargado"}</button>
          </div>`;
...
function renderDelivery() {
  const allRouteClients = clients(true).filter((c) => orderLines(c.id).length);
  const routeClients = allRouteClients.filter((c) => {
    if (state.route.status?.[c.id] === "skip") return false;
  ...
  if (!routeClients.length) {
    $("#view-delivery").innerHTML = `
      <div class="pro-shell narrow-shell">
        <article class="card pro-empty">
          <div class="clean-header"><h2>Reparto</h2><button class="btn soft small" data-action="toggle-delivery-completed">Mostrar completados</button></div>
          <div class="muted" style="margin-top:8px">No quedan clientes pendientes.</div>
        </article>
      </div>`;
    return;
  }
  ...
  $("#view-delivery").innerHTML = `
    <div class="pro-shell delivery-focus">
      <div class="delivery-client-top">
        <div class="order-client-bar top-chooser">
          <button class="btn soft" data-prev-delivery>←</button>
          <select id="deliveryClientSelector" class="search">
            ${routeClients.map((c,i) => `<option value="${i}" ${i===deliveryClientIndex?"selected":""}>${fmt(c.routeOrder)}. ${esc(c.name)}</option>`).join("")}
          </select>
          <button class="btn soft" data-next-delivery>→</button>
        </div>
      </div>

      <article class="card hero-subtle">
        <div class="clean-header">
          <div><h2>${esc(client.name)}</h2><div class="section-subtle">${done}/${lines.length} bajados</div></div>
          <button class="btn soft small" data-complete-client="${client.id}">Todo bajado</button>
        </div>
        <div class="progress" style="margin-top:10px"><span style="width:${pct}%"></span></div>
      </article>
        ...
        <div style="margin-top:2px">
          ${lines.map((line) => {
            ...
            return `<div class="delivery-item ${checked ? "checked" : ""}" data-toggle-delivery="${key}">
              ...
              <div>
                <button class="product-link" data-product-info="${line.product.id}">
                  <span class="product-title item-name">${esc(line.product.name)}</span>
                  <span class="muted">${fmt(finalQty(client.id,line.product.id))} ${esc(line.product.unit)}</span>
                </button>
              </div>
            </div>`;
          }).join("")}
      ...
      <div class="focus-actions">
        <button class="btn soft" data-action="toggle-delivery-completed">${deliveryShowCompleted ? "Ocultar completados" : "Mostrar completados"}</button>
        <button class="btn primary" data-action="final-report">Resumen final</button>
      </div>
    </div>`;
}
...
document.addEventListener("click", async (event) => {
  ...
  if (button.dataset.setPurchaseStatus) {
    ...
    await setPurchaseStatus(productId,status);
    return buyDetailModal(productId);
  }
