...
function renderBuy() {
  ...
  $("#view-buy").innerHTML = `
    <article class="card">
      <div class="clean-header">
        <div><h2>Compra</h2><div class="section-subtle">${list.length} productos</div></div>
    ...
    <article class="card">
      ${list.map((item) => {
        ...
        const totalToBuy = item.qty + extra;
        return `<div class="buy-touch-row">
          <button class="product-link" data-product-info="${item.product.id}">
            <span class="product-title">${esc(item.product.name)}</span>
            <span class="muted">Pedido ${fmt(item.qty)} · Extra ${fmt(extra)} · Comprar ${fmt(totalToBuy)} ${esc(item.product.unit)}</span>
          </button>
...
function buyDetailModal(productId) {
  ...
  openModal("Detalle de compra", `
    <article class="card">
      ...
      <div class="modal-row"><span>Pedido clientes</span><b>${fmt(item.qty)} ${esc(item.product.unit)}</b></div>
      <div class="modal-row"><span>Extra</span><input class="field" style="width:90px" data-extra-input="${productId}" value="${fmt(extra)}"></div>
      <div class="modal-row"><span>Comprado real</span><input class="field" style="width:90px" data-bought-input="${productId}" value="${fmt(bought)}"></div>
      <div class="modal-row"><span>Entregado</span><b>${fmt(delivered)}</b></div>
      <div class="modal-row"><span>Stock restante</span><b>${fmt(stock)}</b></div>
      <button class="btn primary" style="width:100%;margin-top:10px" data-allocate-product="${productId}">Repartir</button>
    </article>
    <article class="card">
