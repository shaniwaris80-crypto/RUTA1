...
function renderHome() {
  ...
  const skipped = allClients.filter((c) => state.route.status?.[c.id] === "skip").length;
  $("#view-home").innerHTML = `
    <article class="card">
      <div class="section-title"><div><h2>Ruta actual</h2><div class="muted">${state.route.date}</div></div></div>
      <div class="stats">
        <div class="summary-mini"><b>${allClients.length}</b><span>Clientes</span></div>
        <div class="summary-mini"><b>${completed}</b><span>Hechos</span></div>
        <div class="summary-mini"><b>${skipped}</b><span>No pide</span></div>
        <div class="summary-mini"><b>${totals().length}</b><span>Productos</span></div>
      </div>
    </article>
    <article class="card">
      <div class="section-title"><h2>Stock final</h2></div>
      <div class="stock-summary">
        <div class="stock-box good"><b>${fmt(purchaseStats().stock)}</b><span>Sobrante</span></div>
        <div class="stock-box bad"><b>${fmt(purchaseStats().shortage)}</b><span>Faltante</span></div>
        <div class="stock-box neutral"><b>${purchaseStats().products}</b><span>Productos</span></div>
      </div>
    </article>
    <article class="card">
      <div class="section-title"><h2>Gestión</h2></div>
      <div class="admin-grid">
        <button class="btn soft" data-action="pending-orders">Clientes pendientes</button>
        <button class="btn soft" data-action="add-client">Añadir cliente</button>
        <button class="btn soft" data-action="add-product">Añadir producto</button>
        <button class="btn soft" data-action="show-history">Historial de rutas</button>
        <button class="btn soft" data-action="export-backup">Exportar copia</button>
        <button class="btn soft" data-action="import-backup">Restaurar copia</button>
        <button class="btn danger" data-action="new-route">Nueva semana/ruta</button>
      </div>
    </article>
    <article class="card deadline-card ${deadlineStatus().ms <= 0 ? "closed" : deadlineStatus().pending.length ? "" : "ok"}">
      <div class="section-title"><div><h2>Plazo</h2><div class="muted">Jueves 23:00</div></div></div>
      <div id="deadlineCountdown" class="deadline-countdown">${formatCountdown(deadlineStatus().ms)}</div>
      <div class="muted">${deadlineStatus().pending.length} pendientes</div>
    </article>`;
}
...
function renderOrders() {
  ...
  if (!client) {
    $("#view-orders").innerHTML = '<article class="card"><div class="muted">No hay clientes pendientes.</div></article>';
    return;
  }
  ...
  $("#view-orders").innerHTML = `
    <div class="orders-shell">
      <aside class="card tablet-client-list">
        <div class="section-title"><h2>Clientes</h2><span class="badge">${allClients.length}</span></div>
        <div class="compact-grid">
          ${allClients.map((c) => {
            const active = c.id === selectedClientId;
            const done = state.route.status?.[c.id] === "done";
            return `<button class="btn ${active ? "dark" : "soft"}" data-open-client="${c.id}" style="text-align:left">
              ${fmt(c.routeOrder)}. ${esc(c.name)} ${done ? "✓" : ""}
            </button>`;
          }).join("")}
        </div>
      </aside>
      ...
      <div>
        ...
        <article class="card clean-section">
          <div class="clean-header">
            <div><h2>${esc(client.name)}</h2><div class="client-progress">${orderLines(client.id).length} productos</div></div>
        ...
        <article class="card">
          <div class="tap-hint">Toca el nombre del producto para ver qué clientes lo llevan.</div>
          <div id="productList" class="compact-product-grid" style="margin-top:8px">
            ${productList.map((p) => productRow(client.id,p)).join("") || '<div class="muted">Sin resultados</div>'}
          </div>
        </article>

        <button class="btn primary big-next" data-save-next="${client.id}|${currentIndex}">Guardar y siguiente</button>
      </div>
    </div>`;
}
...
function renderBuy() {
  ...
  $("#view-buy").innerHTML = `
    <div class="center-card">
      <article class="card">
        <div class="clean-header">
          <div><h2>Compras</h2><div class="section-subtle">Control claro de compra y stock</div></div>
        ...
        <div class="purchase-dashboard" style="margin-top:10px">
          ...
          <div class="purchase-metric"><b>${fmt(stats.ordered)}</b><span>Comprar</span></div>
        ...
        ${filtered.map((item) => {
          ...
          return `<div class="purchase-card">
            <button class="product-link purchase-main" data-product-info="${item.product.id}">
              ...
              <div class="purchase-meta">${stock>0?`Stock ${fmt(stock)}`:stock<0?`Faltan ${fmt(Math.abs(stock))}`:"Sin diferencia"}</div>
            </button>
            <div class="purchase-actions">
              <button class="btn soft small" data-buy-detail="${item.product.id}">Abrir</button>
              <button class="btn ${status==="bought"?"dark":"primary"} small" data-mark-bought="${item.product.id}">${status==="bought"?"Comprado":"Marcar"}</button>
            </div>
          </div>`;
...
function renderLoad() {
  ...
  $("#view-load").innerHTML = `
    <article class="card">
      <div class="clean-header"><div><h2>Carga</h2><div class="section-subtle">${done.length}/${list.length} cargados</div></div><button class="btn soft small" data-action="reset-load">Reiniciar</button></div>
    ...
    <article class="card">
      ${[...pending,...done].map((item) => {
        ...
        return `<div class="load-touch-row">
          ...
          <button class="btn ${checked ? "dark" : "soft"} small" data-toggle-load="${item.product.id}">${checked ? "Cargado" : "Pendiente"}</button>
        </div>`;
...
function renderDelivery() {
  ...
  $("#view-delivery").innerHTML = `
    <div class="delivery-focus">
      ...
      <article class="card">
        <div class="clean-header">
          <div><h2>${esc(client.name)}</h2><div class="client-progress">${done}/${lines.length} bajados</div></div>
